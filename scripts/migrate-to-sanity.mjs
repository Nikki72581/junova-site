/**
 * One-time migration: imports existing articles from articles.ts into Sanity.
 *
 * Prerequisites:
...
// Handles the specific HTML tags used in articles.ts: h2, h3, p, ul/li, ol/li
// with inline marks: strong, em, code
...
function parseInline(html) {
  ...
  while (remaining.length > 0) {
    // strong
    const strong = remaining.match(/^<strong>([\s\S]*?)<\/strong>/);
    if (strong) {
      spans.push({ _type: "span", _key: key("s"), text: decodeEntities(strong[1].replace(/<[^>]+>/g, "")), marks: ["strong"] });
      remaining = remaining.slice(strong[0].length);
      continue;
    }
    // em
    const em = remaining.match(/^<em>([\s\S]*?)<\/em>/);
    if (em) {
      spans.push({ _type: "span", _key: key("s"), text: decodeEntities(em[1].replace(/<[^>]+>/g, "")), marks: ["em"] });
      remaining = remaining.slice(em[0].length);
      continue;
    }
    // code
    const code = remaining.match(/^<code>([\s\S]*?)<\/code>/);
    if (code) {
      spans.push({ _type: "span", _key: key("s"), text: decodeEntities(code[1].replace(/<[^>]+>/g, "")), marks: ["code"] });
      remaining = remaining.slice(code[0].length);
      continue;
    }
    // link
    const link = remaining.match(/^<a[^>]*href="([^"]*)"[^>]*>([\s\S]*?)<\/a>/);
    if (link) {
      const markKey = key("m");
      spans.push({
        _type: "span",
        _key: key("s"),
        text: decodeEntities(link[2].replace(/<[^>]+>/g, "")),
        marks: [markKey],
      });
      // Note: link annotations are referenced by key — Sanity handles them separately
    ...
    if (text) {
      const t = decodeEntities(text[1]);
      if (t) spans.push({ _type: "span", _key: key("s"), text: t, marks: [] });
      remaining = remaining.slice(text[0].length);
      continue;
    }
    // skip unknown tags
    const skip = remaining.match(/^<[^>]+>/);
    if (skip) { remaining = remaining.slice(skip[0].length); continue; }
    break;
  }
...
function block(style, html) {
  const children = parseInline(html);
  if (!children.length) children.push({ _type: "span", _key: key("s"), text: "", marks: [] });
  return {
    _type: "block",
    _key: key("b"),
    style,
...
/** Create a Portable Text list item */
function listItem(listType, html) {
  const children = parseInline(html);
  if (!children.length) children.push({ _type: "span", _key: key("s"), text: "", marks: [] });
  return {
    _type: "block",
...
function htmlToPortableText(html) {
  ...
  while (remaining.length > 0) {
    ...
    if (h2) { blocks.push(block("h2", h2[1])); remaining = remaining.slice(h2[0].length); continue; }

    // h3
    const h3 = remaining.match(/^<h3>([\s\S]*?)<\/h3>/);
    if (h3) { blocks.push(block("h3", h3[1])); remaining = remaining.slice(h3[0].length); continue; }

    // p
    const p = remaining.match(/^<p>([\s\S]*?)<\/p>/);
    if (p) { blocks.push(block("normal", p[1])); remaining = remaining.slice(p[0].length); continue; }

    // ul
    const ul = remaining.match(/^<ul>([\s\S]*?)<\/ul>/);
    if (ul) {
      const items = [...ul[1].matchAll(/<li>([\s\S]*?)<\/li>/g)];
      items.forEach((m) => blocks.push(listItem("bullet", m[1])));
      remaining = remaining.slice(ul[0].length);
      continue;
    }

    // ol
    const ol = remaining.match(/^<ol>([\s\S]*?)<\/ol>/);
    if (ol) {
      const items = [...ol[1].matchAll(/<li>([\s\S]*?)<\/li>/g)];
      items.forEach((m) => blocks.push(listItem("number", m[1])));
      remaining = remaining.slice(ol[0].length);
      continue;
    }

    // skip unknown block-level tags / whitespace
    const skip = remaining.match(/^(<[^>]+>|\s+)/);
    if (skip) { remaining = remaining.slice(skip[0].length); continue; }

    // fallback: treat remaining text as a paragraph
    blocks.push(block("normal", remaining));
    break;
  }
...
// ─── Article data ─────────────────────────────────────────────────────────────
// Copied from articles.ts — the canonical source before Sanity takes over

const articles = [
  {
    slug: "netsuite-saved-searches-guide",
    title: "NetSuite Saved Searches: A Complete Guide",
    description: "Learn how to build powerful saved searches in NetSuite to surface the data you need — from basic filters to complex formulas.",
    category: "NetSuite",
...
<ul>
  ...
  },
  {
    ...
    description: "Understand how NetSuite SuiteFlow workflows work, when to use them, and how to build your first approval workflow.",
    category: "NetSuite",
...
<p>Go to <strong>Customization &gt; Workflow &gt; Workflow Action Errors</strong> to see failed actions. You can also add a <em>Set Field Value</em> action that writes a timestamp to a custom field to trace execution flow.</p>`,
  },
  {
    ...
    description: "A practical guide to understanding NetSuite's role-based access control, how to configure roles, and common pitfalls to avoid.",
    category: "NetSuite",
...
<ul>
  ...
  },
  {
    ...
    description: "Key concepts every Business Central user needs to know — dimensions, posting groups, number series, and the general ledger setup.",
...
<ul>
  ...
  },
  {
    ...
    description: "Where AI actually delivers value in ERP workflows today — from anomaly detection to intelligent data entry — and how to evaluate what's hype vs. real.",
...
<p>Even the best AI makes mistakes. Every AI-driven workflow needs a clear exception path — a queue, an alert, or a dashboard where humans review edge cases. Removing humans entirely from high-stakes processes is where AI projects fail.</p>`,
  },
  {
    ...
    description: "Learn how Acumatica's customization framework works — from screen modifications in the Customization Editor to business event rules and low-code automation.",
    category: "Acumatica",
...
<ul>
  ...
  },
  {
    ...
    description: "How to evaluate ERP options without getting sold. A practical framework for requirements gathering, vendor assessment, and avoiding common selection mistakes.",
...
async function run() {
  console.log(`\nConnecting to Sanity project: ${process.env.NEXT_PUBLIC_SANITY_PROJECT_ID}`);
  console.log(`Dataset: ${process.env.NEXT_PUBLIC_SANITY_DATASET ?? "production"}\n`);

  if (!process.env.NEXT_PUBLIC_SANITY_PROJECT_ID || process.env.NEXT_PUBLIC_SANITY_PROJECT_ID === "your-project-id-here") {
    console.error("❌  Set NEXT_PUBLIC_SANITY_PROJECT_ID in .env.local before running this script.");
    process.exit(1);
  }
  if (!process.env.SANITY_API_TOKEN || process.env.SANITY_API_TOKEN === "your-write-token-here") {
    console.error("❌  Set SANITY_API_TOKEN in .env.local (needs Editor permissions).");
    process.exit(1);
  }
  ...
  console.log(`\n✅  Migration complete — ${articles.length} articles imported into Sanity.\n`);
  console.log("   Open your Studio at /studio to review the content.\n");
}
